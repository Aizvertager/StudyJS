let start = document.querySelector('#start'),
    ...
    getAddExpenses = additionalExpensesItems.value.split(','),
...
AppData.prototype.start = function() {
    ...
    this.getBudget();
    this.getAddBlock(getAddExpenses, appData.addExpenses);
    this.getAddBlock(additionalIncomeItems, appData.addIncome);
...
//     });
// };
AppData.prototype.getAddBlock = function(elem, elem1) {
    let _this = this;

    elem.forEach( (item) => {
        if(elem === additionalIncomeItems) {
            item = item.value.trim();
        }
        item = item.trim();
        if(item !== '') {
...
AppData.prototype.eventsListeners = function() {
    ...
    document.querySelector('.btn_plus.income_add').addEventListener('click', () => {
        _this.addBlock('.income-items', '.btn_plus.income_add', '.income-items');
    });
    document.querySelector('.btn_plus.expenses_add').addEventListener('click', () => {
        _this.addBlock('.expenses-items', '.btn_plus.expenses_add', '.expenses-items');
    });
